...
export default function ViewProduct() {
    ...
    async function view_user() {
        ...
        makeGetRequest(`/manageproduct?page=${page}&perpage=${perpage}&delay=1`,bodyFormData).then((response)=> {
    ...
    async function searchuser() {
        let bodyFormData=new FormData();
        makeGetRequest(`/serachproduct?pages=${pages}&perpage=${perpage}&find=${find}&delay=1`,bodyFormData).then((response)=> {
            if(response.data.status === "1") {
    ...
    {
      let bodyFormData=new FormData();
      makePostRequest(`/deleteproduct/${cid}`,bodyFormData).then((response)=> {
          if(response.data.status === "1") {
        ...
        cell: row => {
            return (
              <div className="tblImgOut"> 
                <img src={process.env.PUBLIC_URL + `/uploads/products/${row.img}`} alt="img" className="readndrollimg"/> 
              </div>
            )
        }
    },
    ...
    },
    {
        ...
        cell:row=> {
            ...
            <div className="tabButOut">
                <Button onClick={()=>history.push("/editproduct/"+row.pid)} startIcon={<ModeEditOutlineIcon/>}/>
                <Button onClick={()=>handleClickOpen(row.pid)}  startIcon={<DeleteIcon/>}/>
            </div>
            )
        }
    },
  ...
  return (
    ...
    <Box>
        ...
        <Box className='align'>
            ...
            <Button className="btn-manage"  variant="contained" color="primary" onClick={()=>history.push("/addproduct")}>
             Add New Product
             </Button>
        </Box>
        <div className='tbl'>
